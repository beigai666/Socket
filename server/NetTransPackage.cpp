...
NetTransPackage* getPackage(NetTransPackage& temp) {
    uint16_t length = 0;
    GET_UINT16_BE(length, temp.Len);
    NetTransPackage* pack = (NetTransPackage*)malloc(sizeof(NetTransPackage) +length);
    PUT_UINT16_BE(length, pack->Len);
    MOV_UINT16_BE(temp.CRC, pack->CRC);
    return pack;
}